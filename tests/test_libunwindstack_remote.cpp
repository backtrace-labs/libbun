...
TEST(libunwindstack_remote, parent_unwinds_child) {
...
TEST(libunwindstack_remote, child_unwinds_parent) {
	std::vector<char> buf(0x10000);
	struct bun_handle handle;
	struct bun_buffer buffer;
	...
	signal(SIGUSR1, +[](int){ initialized.store(true); });
	signal(SIGUSR2, +[](int){ finished.store(true); });
	pid_t original_pid = getpid();
	pid_t forked = fork();
	if (forked == -1) {
		FAIL();
	...
	size_t size = bun_unwind_remote(&handle, &buffer, original_pid);

	int status;
	kill(original_pid, SIGUSR2);
	// waitpid(forked, &status, 0);
